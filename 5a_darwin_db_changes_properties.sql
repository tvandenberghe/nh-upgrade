...
INSERT INTO TAG_AUTHORITY (domain_ref,source,url,urn,code,type,pref_label,definition) values ((select id from authority_domain where name='nerc.ac.uk'),null,'http://vocab.nerc.ac.uk/collection/P01/current/SPWGXX01/','SDN:P01::SPWGXX01','Weight',ARRAY['P01'],'Specimen weight of biological entity specified elsewhere','The weight of biological specimens identified elsewhere in the data or metadata. The method used to determine specimen weight should be recorded in the usage metadata.') on conflict (domain_ref, url) do nothing returning id; 
INSERT INTO TAG_AUTHORITY (domain_ref,source,url,urn,code,type,pref_label,definition) values ((select id from authority_domain where name='nerc.ac.uk'),null,'http://vocab.nerc.ac.uk/collection/P01/current/OBSINDLX/','SDN:P01::OBSINDLX','Length',ARRAY['P01'],'Length of biological entity specified elsewhere','The mean length of specimens of a biological object (identified elsewhere in the data or metadata) in the sample or observation. This may refer to a single individual if the count is one.') on conflict (domain_ref, url) do nothing returning id; 
...
insert into property_tag_authority (tag_authority_ref, tag_authority_match_predicate, property_type_id) select ta.id, 'skos:exactMatch', pt.id from tag_authority ta, property_type pt where ta.urn='SDN:P01::SPWGXX01' and pt.type = 'body_weight' ON CONFLICT (tag_authority_ref, property_type_id, tag_authority_match_predicate) DO NOTHING;
insert into property_tag_authority (tag_authority_ref, tag_authority_match_predicate, property_type_id) select ta.id, 'skos:exactMatch', pt.id from tag_authority ta, property_type pt where ta.urn='SDN:P01::OBSINDLX' and pt.type = 'body_length' ON CONFLICT (tag_authority_ref, property_type_id, tag_authority_match_predicate) DO NOTHING;
