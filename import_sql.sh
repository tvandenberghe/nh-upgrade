...
#psql -h "$server" -d "$db" -U postgres -c "DROP TABLE IF EXISTS import.taxamatch cascade;"
...
#DROP MATERIALIZED VIEW IF EXISTS darwin2.mv_darwin_ipt_rbins_mof CASCADE;
...
#DROP MATERIALIZED VIEW IF EXISTS darwin2.mv_spatial, darwin2.mv_eml, darwin2.mv_eml_marine, darwin2.mv_tag_to_country, darwin2.mv_tag_to_locations, darwin2.mv_darwin_ipt_rbins;
...
echo "3-->MODIFY TAXONOMY TABLES"
#psql -h "$server" -d "$db" -U postgres -q  < 3_darwin_db_changes_tables_taxonomy.sql 
echo "4-->CREATE SPATIAL VIEWS AND FUNCTIONS"
#psql -h "$server" -d "$db" -U postgres -q  < 4_darwin_db_changes_functions_views.sql
echo "5a-->CREATE TABLES TO BETTER STORE PROPERTIES"
#psql -h "$server" -d "$db" -U postgres -q  < 5a_darwin_db_changes_properties.sql
...
psql -h "$server" -d "$db" -U postgres -q  < 7_ipt_views_per_dataset.sql
