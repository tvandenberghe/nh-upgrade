psql -h localhost -d darwin2_rbins_test -U postgres -c "DROP TABLE IF EXISTS import.taxamatch;"

pgfutter --table "taxamatch" --schema "import" --db darwin2_rbins_test  --host localhost --port 5432 --user postgres --pw "postgres" csv /home/thomas/Documents/Project-NaturalHeritage/mappings/taxamatch.csv -d ','

psql -h localhost -d darwin2_rbins_test -U postgres -c "
...
psql -h localhost -d darwin2_rbins_test -U postgres -q < 1_darwin_db_changes_tables.sql
echo "2a-->ADD TAG AUTHORITY TABLE"
psql -h localhost -d darwin2_rbins_test -U postgres -q  < 2a_darwin_db_changes_tag_authority.sql
echo "3a-->POPULATE AUTHORITY COUNTRIES"
psql -h localhost -d darwin2_rbins_test -U postgres -q  < 3a_darwin_db_changes_tag_authority_data_countries.sql 
echo "3c-->POPULATE AUTHORITY LOCALITIES"
psql -h localhost -d darwin2_rbins_test -U postgres -q  < 3c_darwin_db_changes_tag_authority_data.sql
echo "4-->MODIFY TAXONOMY TABLES"
psql -h localhost -d darwin2_rbins_test -U postgres -q  < 4_darwin_db_changes_tables_taxonomy.sql 
echo "5-->CREATE EML VIEW"
psql -h localhost -d darwin2_rbins_test -U postgres -q  < 5_darwin_db_changes_functions_eml_views.sql
echo "6-->CREATE IPT OCCURRENCE VIEW"
psql -h localhost -d darwin2_rbins_test -U postgres -q  < 6_darwin_db_changes_ipt_view.sql
echo "7-->CREATE TABLES TO BETTER STORE PROPERTIES"
psql -h localhost -d darwin2_rbins_test -U postgres -q  < 7_darwin_db_changes_properties.sql
echo "8-->CREATE SAMPLING VIEW"
psql -h localhost -d darwin2_rbins_test -U postgres -q  < 8_darwin_db_changes_sampling_view.sql
echo "9-->CREATE MOF VIEW"
psql -h localhost -d darwin2_rbins_test -U postgres -q  < 9_darwin_db_changes_ipt_mof_view.sql
