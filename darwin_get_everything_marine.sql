...
drop MATERIALIZED view IF EXISTS select_eml_marine;
create MATERIALIZED VIEW select_eml_marine as
...
select 'The '||c.title_en || ' contains ' || si.nb_spec ||' digitised specimens over '||sc.nb_species||' taxa (mostly species). This dataset only contains the marine species of the collection. The following classes are included: '||coc.CLASS_TAXONOMIC_COVERAGE||'.' as abstract, 
curator.family_name ||', '||left(curator.given_name,1) ||'., Semal, P. (2018) '||c.title_en||'. Dataset/Occurence' as citation, 'collection' as scope, si.nb_spec,sc.nb_species,si.ig_num, c.id,c.name, c.name_indexed as code,c.title_en,c.title_nl,c.title_fr, gc.GEOGRAPHIC_COVERAGE,gc.min_lon,gc.max_lon,gc.min_lat,gc.max_lat,
case when tc.date_from='xxxx-xx-xx' then null else tc.date_from end as START_DATE, 
case when tc.date_to='xxxx-xx-xx' then null else tc.date_to end as end_DATE, coc.CLASS_TAXONOMIC_COVERAGE, coc.order_TAXONOMIC_COVERAGE, 'natural history collection, RBINS, DaRWIN, '||name as keywords, 'Royal Belgian Institute for Natural Sciences' as institute_name, 
'Department of Scientific Heritage Service' institute_dept_abbrev, curator.given_name ||' '||curator.family_name as boss,uc1.entry as boss_email, 'curator' as boss_role,staff.given_name ||' '||staff.family_name as subboss,uc2.entry as subboss_email, 'Collection manager' as subboss_role from collections c
...
where c.publish_to_gbif=true and c.profile @> array['isMarine']
